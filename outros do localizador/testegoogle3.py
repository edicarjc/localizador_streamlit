...
st.set_page_config(page_title="Localizador de Técnicos (v2.1 Otimizado)", layout="wide")
                ...
                with st.spinner(f"Buscando técnicos a até {st.session_state.raio_selecionado} km..."):
                    ...
                    if tecnicos_proximos is not None and not tecnicos_proximos.empty:
                        ...
                        with col_mapa:
                            ...
                            # --- CÓDIGO DO MAPA (COM CORREÇÃO DE ESTILO) ---
                            ...
                            # 5. Renderizar o mapa com o estilo 'light' (RESOLVE O PROBLEMA DE CARREGAMENTO)
                            st.pydeck_chart(pdk.Deck(
                                map_style='light', 
                                initial_view_state=view_state,
                                layers=[camada_tecnicos, camada_cliente], 
                            ))
                            # ------------------------------------------------
                        ...
                        with col_lista:
                            ...
                            # Lista formatada em colunas (INCLUSÃO DO COORDENADOR AQUI)
                            cols_tecnicos = st.columns(2)
                            
                            for i, row in tecnicos_proximos.reset_index(drop=True).iterrows():
                                with cols_tecnicos[i % 2]:
                                    st.markdown(f"**{row['tecnico']}** - {row['cidade']}/{row['uf']}")
                                    st.write(f"Coordenador: **{row['coordenador']}**") # <<< LINHA ADICIONADA
