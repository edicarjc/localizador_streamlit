...
st.set_page_config(page_title="Localizador de Técnicos (v2.13 Mapa st.map Corrigido)", layout="wide") 
...
def processar_chamados_em_lote(df_chamados, df_tecnicos_base, api_key, max_distance_km, capacidade_diaria):
    ...
    for index, row_chamado in df_chamados.iterrows():
        ...
        # CUSTO ATUALIZADO: Multiplicar por CUSTO_POR_KM (que agora é 2.0)
                ...
                with st.spinner(f"Buscando técnicos a até {st.session_state.raio_selecionado} km..."):
                    ...
                    if tecnicos_proximos is not None and not tecnicos_proximos.empty:
                        ...
                        with col_mapa:
                            ...
                            # --- CÓDIGO DO MAPA (MUDANÇA AQUI: USO DE st.map() com colunas de cor/tamanho) ---
                            
                            # 1. Preparar dados
                            # Renomear colunas para o padrão de st.map() (lat, lon)
                            df_mapa = tecnicos_proximos[['latitude', 'longitude', 'tecnico']].rename(
                                columns={'latitude': 'lat', 'longitude': 'lon'}
                            )
                            # Adicionar a localização do cliente (ponto de origem)
                            df_cliente_mapa = pd.DataFrame([localizacao_cliente]).rename(columns={'lat': 'lat', 'lng': 'lon'})
                            df_cliente_mapa['tecnico'] = 'CLIENTE (Origem)' # Adiciona label para diferenciar
                            
                            df_mapa_completo = pd.concat([df_mapa, df_cliente_mapa])
                            
                            # **CORREÇÃO: Criar colunas de cor e tamanho no DataFrame**
                            
                            # Adiciona a coluna de cores (HEX strings)
                            df_mapa_completo['color_code'] = np.where(
                                df_mapa_completo['tecnico'] == 'CLIENTE (Origem)', 
                                '#FF0000', # Vermelho para Cliente
                                '#0066CC'  # Azul para Técnicos
                            )
                            
                            # Adiciona a coluna de tamanho (pixels)
                            df_mapa_completo['size_px'] = np.where(
                                df_mapa_completo['tecnico'] == 'CLIENTE (Origem)', 
                                15, # Cliente maior
                                5   # Técnicos menor
                            )
                            
                            # 2. Definir o centro do mapa (Localização do Cliente)
                            center_lat = localizacao_cliente['lat']
                            center_lon = localizacao_cliente['lng']
                            
                            # 3. Renderizar o mapa usando st.map(), passando o NOME da coluna
                            st.map(
                                df_mapa_completo, 
                                latitude='lat', 
                                longitude='lon',
                                color='color_code', # Passa o NOME da coluna de cores
                                size='size_px',     # Passa o NOME da coluna de tamanho
                                zoom=10
                            )
                            # ------------------------------------------------
                        ...
                        with col_lista:
                            ...
                            # Informação sobre o cálculo do custo
                            ...
                            df_to_export.rename(columns={
                                ...
                                # Atualizar o label da coluna na exportação
    ...
    if st.session_state.editor_authenticated:
        ...
        if not df_editable.empty:
            ...
            with col_export:
                ...
                df_final_export = st.session_state.df_tecnicos 
        ...
        # Se não autenticado no editor, não mostra o editor, mas também não usa st.stop()
        pass
...
with tab4:
    ...
    # INFORMAÇÃO DO CUSTO ATUALIZADA
